...
var setPlugin = function(root) {
	...
	MIDI.noteOnName = root.noteOnName;
	MIDI.noteOffName = root.noteOffName;
...
if (window.AudioContext || window.webkitAudioContext) (function () {
	...
	root.noteOffName = function (channel, name, delay) {
		var note = 0;
		for (var i = 21; i < 109; i++ ) {
			if(audioBuffers["0"+i].id == name) {
				note = i;
				break;
			}
		}
	...
	root.noteOnName = function (channel, name, velocity, delay) {
		/// check whether the note exists
		if (!MIDI.channels[channel]) return;
		var instrument = MIDI.channels[channel].instrument;
		var note = 0;
